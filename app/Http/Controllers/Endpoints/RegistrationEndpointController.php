...
class RegistrationEndpointController extends Controller implements HasMiddleware
{
    ...
    public function update(UpdateTournamentRegistrationRequest $request, Registration $registration)
    {
        if ($request->has('opens_at')) {
            $registration->opens_at = Carbon::createFromFormat('n-j-Y', $request->opens_at);
        }
        if ($request->filled('closes_at')) {
            $registration->closes_at = Carbon::createFromFormat('n-j-Y', $request->closes_at);
        } else {
            $registration->closes_at = null;
        }
